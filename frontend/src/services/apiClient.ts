...
export const apiClient = {
  ...
  createClient: async (email: string, company?: string, name?: string, phone?: string) => {
    const response = await axiosInstance.post('/clients', { email, company, name, phone });
  ...
  getProgress: async (uuid: string) => {
    const response = await axiosInstance.get(`/clients/${uuid}/progress`);
    return response.data as OnboardingProgress;
  },

  // Submit step data
  submitStep: async (uuid: string, step: number, formData: StepFormData) => {
    const response = await axiosInstance.post(`/clients/${uuid}/step/${step}`, { formData });
    return response.data;
  }
};
